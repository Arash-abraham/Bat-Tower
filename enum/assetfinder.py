...
def assetfinder(domain):
    ...
    command = f"assetfinder {domain} -subs"
    print(f"{colors.GRAY}Executing commands: {command}{colors.RESET}")
    res = run_command_in_zsh(command)

    res_num = len(res) if res else 0
    print(f"{colors.GRAY}The number of subdomains that assetfinder found for {domain}, results: {res_num}{colors.RESET}")
...
if __name__ == "__main__":
    ...
    if domain is False:
        print(f"Usage: bat_assetfinder domain")
        sys.exit()
